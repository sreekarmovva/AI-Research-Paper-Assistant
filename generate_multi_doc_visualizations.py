...
def generate_multi_doc_visualizations():
    ...
    plt.title('Multi-Doc: Average BERTScore (F1) Comparison', pad=15)
    plt.ylabel('BERTScore F1 (0 - 1.0)')
    plt.ylim(0, 1.0)
    for container in ax.containers:
        ax.bar_label(container, fmt='%.3f', padding=3)
    plt.tight_layout()
    plt.savefig('multi_doc_chart_bertscore_comparison.png', dpi=300)
    plt.close()
    ...
    plt.title('Multi-Doc: LLM-as-a-Judge Evaluation Metrics (Average)', pad=15)
    ...
    plt.tight_layout()
    plt.savefig('multi_doc_chart_llm_metrics_comparison.png', dpi=300)
    plt.close()
    ...
    plt.title('Multi-Doc: Average Latency Comparison', pad=15)
    plt.ylabel('Latency (Seconds)')
    for container in ax3.containers:
        ax3.bar_label(container, fmt='%.2fs', padding=3)
    plt.tight_layout()
    plt.savefig('multi_doc_chart_latency_comparison.png', dpi=300)
    plt.close()

    print("\n✅ Multi-Doc visualization tasks complete!")
    print(f"-> Saved: multi_doc_chart_bertscore_comparison.png")
    print(f"-> Saved: multi_doc_chart_llm_metrics_comparison.png")
    print(f"-> Saved: multi_doc_chart_latency_comparison.png")

if __name__ == "__main__":
    generate_multi_doc_visualizations()
